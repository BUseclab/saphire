...
PHP_MINIT_FUNCTION(xdebug)
{
	...
	int err = 0;
	shm_pid = getpid();
	sprintf(shm_name, "%s_%d", name, shm_pid);
	shm_fd = shm_open(shm_name, O_CREAT | O_RDWR , 0777);
	err = ftruncate(shm_fd, SHM_SIZE);
	if(err)
	{
	  zend_error(E_WARNING, "Failed to truncate the shared memory region ");
	}
	shm_ptr = mmap( 0, SHM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
	fFile = fopen("/tmp/funcs", "a");
...
void confirm_shm_pid()
{
	...
	((char*)shm_ptr)[SHM_FORK_FLAG_OFFSET] -= 1;
	shm_pid = getpid();
	munmap(shm_ptr, SHM_SIZE);
	sprintf(shm_name, "%s_%d", name, shm_pid);
	shm_fd = shm_open(shm_name, O_CREAT | O_RDWR , 0777);
...
void xdebug_execute_internal(zend_execute_data *current_execute_data, zval *return_value)
{
	...
	fflush(fFile);
	while(update_shared_callstack(shm_ptr, fFile) == -1)
	{
		confirm_shm_pid();
	}
