...
int update_shared_callstack(void* ptr, FILE* fFile TSRMLS_DC)
{
	...
	if ( ptr != NULL)
	{
		...
		int size = 0;
		int line_size = 0;
		char tmp[SHM_CALLSTACK_SIZE];
		if(((char*)ptr)[SHM_FORK_FLAG_OFFSET] > 0)
		{
			return -1;
		}
		if ( XG(stack) && XG(stack)->size)
		{
			for ( le = XDEBUG_LLIST_TAIL(XG(stack)); le != NULL && size < SHM_CALLSTACK_SIZE ; le = XDEBUG_LLIST_PREV(le))
			{
				i = XDEBUG_LLIST_VALP(le);
				tmp_name = i->function.function;
				if(i->function.class != NULL) 
				{
					sprintf(tmp," > %s::%s ->%s:%d\n", i->function.class, tmp_name, i->filename, i->lineno);
					line_size = strlen(tmp);
				}
				else
				{
					sprintf(tmp," > %s ->%s:%d\n", tmp_name, i->filename, i->lineno);
					line_size = strlen(tmp);
				}
				if ( size + line_size< SHM_CALLSTACK_SIZE )
				{
					sprintf(ptr, "%s", tmp);
					ptr += line_size;
					size += line_size;
		...
		((char*)base_ptr)[SHM_CALLSTACK_SIZE-1] = '\0';
	}
	return 0;
}
